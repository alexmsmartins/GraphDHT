...
/**
 * Here is where we define the core protocol that simulates the behaviour of a
 * chord application in every node.
 * Messages will be exchanged by the transport protocol defined above.
 *
 *
 * TODO
 * Aqui fica a maior parte do nosso trabalho.
 * Aqui está o chord implementado.
 * Vou ter de trabalhar bem isto.
 *
 *
 * @author Andrea
 * 
...
public class ChordProtocol implements EDProtocol {
     ...
     * network, and the protocols in them. Each node has the same kinds of
     * protocols; that is, instances of a protocols form an array in the
     * network, with one instance in each node. The instances of the nodes and
     * the protocols are created by cloning. That is, only one instance is
     * constructed using the constructor of the object, which serve as
     * prototypes, and all the nodes in the network are cloned from this
     * prototype. For this reason, it is very important to play attention to the
     * implementation of the cloning method of the protocols.
     *
     * 
     * @return
     */
