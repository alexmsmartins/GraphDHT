...
public class CreateNetwork implements Control {
    ...
    public Node findId(BigInteger id, int nodeOne, int nodeTwo) {
        ...
        try {
            BigInteger newId = ((ChordProtocol) ((Node) Network.get(middle)).getProtocol(pid)).chordId;
            BigInteger lowId;
            if (middle > 0) {
                lowId = ((ChordProtocol) ((Node) Network.get(middle - 1)).getProtocol(pid)).chordId;
            } else {
                lowId = newId;
            }
            BigInteger highId = ((ChordProtocol) ((Node) Network.get(middle + 1)).getProtocol(pid)).chordId;
