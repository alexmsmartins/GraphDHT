...
import peersim.chord.NodeComparator;
 ...
 * of entries) and other parameters.
 *
 *
 *
 * @author Andrea
 * 
 */
public class CreateGraph implements Control {
     ...
     *
     */
    public CreateGraph(String prefix) {
    ...
    public boolean execute() {
        for (int i = 0; i < Network.size(); i++) {
            ...
            GraphDHTProtocol cp = (GraphDHTProtocol) node.getProtocol(pid);
            cp.m = idLength;
    ...
    public Node findId(BigInteger id, int nodeOne, int nodeTwo) {
        ...
        try {
            BigInteger newId = ((GraphDHTProtocol) (Network.get(middle)).getProtocol(pid)).chordId;
            BigInteger lowId;
            if (middle > 0) {
                lowId = ((GraphDHTProtocol) (Network.get(middle - 1)).getProtocol(pid)).chordId;
            } else {
                lowId = newId;
            }
            BigInteger highId = ((GraphDHTProtocol) (Network.get(middle + 1)).getProtocol(pid)).chordId;
    ...
    public void createFingerTable() {
        BigInteger idFirst = ((GraphDHTProtocol) Network.get(0).getProtocol(pid)).chordId;
        BigInteger idLast = ((GraphDHTProtocol) Network.get(Network.size() - 1).getProtocol(pid)).chordId;
        for (int i = 0; i < Network.size(); i++) {
            Node node = Network.get(i);
            GraphDHTProtocol cp = (GraphDHTProtocol) node.getProtocol(pid);
            ...
            if (i > 0) {
                cp.predecessor = Network.get(i - 1);
            } else {
                cp.predecessor = Network.get(Network.size() - 1);
            }
            int j = 0;
