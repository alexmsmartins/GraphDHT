...
public class TestRunner {
    ...
    public double Run() {
        ...
        for (Object parameterNameObject : parameters.keySet()) {
            ...
            String parameterValue = parameters.getProperty(parameterName).trim();
            ...
            if (parameterName.startsWith(testIdentifier + ".")) {
                parameterName = parameterName.substring(testIdentifier.length() + 1);
            }
        ...
        if (reportersString != null) {
            ...
            for (String reporterIdentifier : reporterIdentifiers) {
                Reporter reporter = Benchmark.getReporter(reporterIdentifier.trim());
                if (reporter == null) {
                    throw new RuntimeException(
        ...
        for (Reporter reporter : reporters) {
            System.out.println(" Preparing report " + reporter.getIdentifier());
            try {
        ...
        for (int variableParameterIndex = 0; variableParameterIndex < variableParameterVariations
            ...
            for (String parameterName : variableParameters.keySet()) {
                testParameters.put(parameterName, variableParameters.get(parameterName)[variableParameterIndex]);
            }
